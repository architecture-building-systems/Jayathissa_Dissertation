...
% To study the electricity generation and building energy consumption, a 3D geometry of the room and solar facade is built using the Rhinoceros software \cite{Rhino}, and its parametric modelling plugin Grasshopper \cite{grasshopper}. The solar facade consists of 400mm CIGS square panels that can rotate in two degrees of freedom. On the horizontal axis, the panels can move from 0$^{\circ}$ (closed) to 90$^{\circ}$ (open) position in steps of 22.5$^{\circ}$. In the vertical axis, it can move from 45$^{\circ}$ to -45$^{\circ}$ in 22.5$^{\circ}$ steps. Existing ASF systems \cite{nagy2016adaptive} have independently actuated panels and a continuous range of actuation, however for simplicity, we group all panels into one cluster that moves in unison. This leaves us with 25 possible dynamic configurations of the facade system. A corresponding workflow can be seen in Figure \ref{fig:workflow2}. 
...
%We have developed a framework to study both the electricity energy harvest of dynamic BIPV modules, and its influence on the building system through shading. A high resolution radiation analysis is conducted in the Grasshopper/Rhino environment \cite{grasshopper} using the LadyBug plug-in \cite{roudsari2013ladybug}. The results from the radiation analysis are fed through an electrical simulation which calculates the PV production while taking into account effects of module self-shading \cite{hofer2016parametric}. The solar gains through the window are coupled to a resistance - capacitance (RC) network \cite{de2008iso} to model the building energy demand. This enables us to evaluate the energetic performance of an adaptive solar facade, in a set configuration for a single time step. 


%An optimisation algorithm then finds the combination that will minimise the net building energy demand, i.e. the difference of building energy demand for heating, cooling, lighting against BIPV electricity generation. Transient elements of the results, such as the temperature of the room, and thermal control settings are then stored for the next time step. 


% An optimisation algorithm then finds the combination that will minimise the net building energy demand according to Equation \ref{eq:min}, i.e. the difference of building energy demand for heating, cooling, lighting and BIPV electricity generation. Transient elements of the results, such as the temperature of the room, and thermal control settings are then stored for the next time step. 


%Figure \ref{fig:workflow2} visualises the process flow. Each method in this process flow are expaned in the following subsections.

%The following sub-sections will detail each of these methods as visualised in Figure \ref{fig:workflow2}. 

% \begin{figure}
% \begin{center}
% \includegraphics[width=0.5\columnwidth, trim= 0cm 0cm 0cm 0cm,clip]{workflow_vertical.pdf} 
